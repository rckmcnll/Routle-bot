...
from routle_bot import login, poll, run, BOT_HANDLE, BOT_PASSWORD
...
def main():
    print(f"🤖 Bot started")
    print(f"   Polling feed every {POLL_INTERVAL_MINUTES} minute(s) for new results")
    print(f"   Daily leaderboard  → every day at {LEADERBOARD_TIME}")
    print(f"   Weekly leaderboard → {['Mon','Tue','Wed','Thu','Fri','Sat','Sun'][WEEKLY_LEADERBOARD_DAY]}s at {LEADERBOARD_TIME}")
    print(f"   Monthly leaderboard → 1st of each month at {LEADERBOARD_TIME} (posts previous month)")
    print(f"   Yearly leaderboard  → Jan 1st at {LEADERBOARD_TIME} (posts previous year)")
    ...
    while True:
        ...
        if last_poll_time is None or (now - last_poll_time) >= poll_interval:
            last_poll_time = now
            print(f"[{now.strftime('%H:%M:%S')}] Polling feed...")
            try:
                new = poll(session=session)
                if new:
                    print(f"  → {new} new result(s) processed.")
                else:
                ...
                try:
                    ...
                    print("  ❌ Re-auth also failed. Will retry next tick.")

        # ── Fire leaderboards on schedule ──────────────────────────────────────
        for period in ("daily", "weekly", "monthly", "yearly"):
            if _should_fire(now, last_leaderboard_fired, period):
                ref = _ref_date_for(period, now)
                print(f"\n🔔 [{now.strftime('%H:%M')}] Firing {period} leaderboard (ref={ref})")
                try:
                    # run() re-collects + posts leaderboard; pass existing session
                    run(post_date=ref, period=period)
                except Exception:
                    print(f"❌ {period} leaderboard failed:")
                    traceback.print_exc()
